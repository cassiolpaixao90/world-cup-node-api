...
exports.authenticate = async (data, req) => {
    try {
        ...
        if (!await existingUser.passwordIsValid(data.password)) {
            ...
            throw new WorldCupError(messageProperties.MESSAGE_USUARIO_INVALIDO, 404);
        }
        if( !await Login.canAthenticate(identityKey)){
            throw new WorldCupError(messageProperties.MESSAGE_CONTA_TEMP_BLOQ, 500);
        }
        if( await Login.inProgress(identityKey)){
            throw new WorldCupError(messageProperties.MESSAGE_USUARIO_AUTH, 500);
        }
...
exports.refreshToken = async (token) => {
    try {
        ...
        if (!user) {
            throw new WorldCupError(messageProperties.MESSAGE_TOKEN_NAO_ENCONTRADO, 404);
        }
        return await generateToken({
            id: user._id,
            email: user.email,
